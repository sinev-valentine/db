...
namespace fc_light
{
   ...
   void from_variant( const variant& var,  optional<T>& vo )
   {
    ...
    template<size_t N>
    void to_variant( const uint8_t (&arr)[N],  variant& vo )
    {
        std::vector<char> buf(N);
        const char* inP_ptr = reinterpret_cast<const char*>(&arr[0]);
        std::copy(inP_ptr, inP_ptr + N, buf.data());
        vo = variant(buf);
    }

    template<size_t N>
    void from_variant( const variant& var, uint8_t (&arr)[N]   )
    {
        std::vector<char> vo;
        from_variant( var,  vo );
        std::copy(vo.begin(), vo.end(), &arr[0]);
    }

    template<size_t N>
    void to_variant( const uint32_t (&arr)[N],  variant& vo )
    {
        size_t size = sizeof(uint32_t)*N;
        std::vector<char> buf(size);
        const char* inP_ptr = reinterpret_cast<const char*>(&arr[0]);
        std::copy(inP_ptr, inP_ptr + size, buf.data());
        vo = variant(buf);
    }

    template<size_t N>
    void from_variant( const variant& var, uint32_t (&arr)[N]   )
    {
        std::vector<char> vo;
        from_variant( var,  vo );
        std::copy(vo.begin(), vo.end(), reinterpret_cast<char*>(&arr[0]));
    }
